...
export function TopBar({
  ...
  const gateClips = clips.filter((c) => c.source !== "contoh");
  const sampleClips = clips.filter((c) => c.source === "contoh");
  return (
    <header className="glass-card px-3 py-2 flex flex-wrap items-center gap-x-3 gap-y-2">
      ...
      <div className="flex items-center gap-2 ml-auto flex-wrap">
        {/* Grouped, not merged. "Semua klip" runs this gate's own footage only:
            sweeping the reference set into it would file other people's trucks
            as crossings at this gate. A sample has to be picked by name. */}
        <select
          value={selectedClip}
        ...
        >
          <option value="ALL">Semua klip gate ({gateClips.length})</option>
          {gateClips.length > 0 && (
            <optgroup label="Klip gate ini">
              {gateClips.map((c) => (
          ...
          {sampleClips.length > 0 && (
            <optgroup label="Video contoh (bukan armada ini)">
              {sampleClips.map((c) => (
                <option key={c.name} value={c.name}>{c.name}</option>
              ))}
            </optgroup>
